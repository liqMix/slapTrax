...
func (m *Manager) Initialize() error {
	...
	if settings, err := m.storage.LoadSettings(); err == nil {
		...
		m.currentUser.Settings.IsNewUser = false
	}
...
func (m *Manager) PlayOffline() {
	m.currentUser.Settings.IsGuest = true
	m.currentUser.Username = l.String(l.GUEST)
	m.loginState = StatePlayOffline
}
...
func (m *Manager) Register(username, password string) error {
	...
	if err := m.storage.client.Register(username, password, m.currentUser.Settings); err != nil {
...
func (m *Manager) GetLeaderboard(song, difficulty string) ([]Score, error) {
	if !m.HasConnection() {
		return []Score{}, nil
	}
	return m.storage.client.GetLeaderboard(song, difficulty)
}
...
func (m *Manager) Login(username, password string, remember bool) error {
	...
	m.session = &Session{
		...
		CreatedAt:    time.Now(),
		ExpiresAt:    time.Now().Add(24 * time.Hour), // TODO: Get from token
	}
	...
	m.currentUser = user
	m.currentUser.Settings.IsGuest = false
...
func (m *Manager) TryAutoLogin() error {
	...
	logger.Debug("storage: %v", m.storage)
	logger.Debug("client: %v", m.storage.client)
	logger.Debug("creds: %v", creds)
...
func (m *Manager) SaveSettings() error {
	...
	if m.loginState == StateOnline {
		if err := m.storage.client.UpdateUser(m.session.AccessToken, map[string]interface{}{
			"settings": m.currentUser.Settings,
		}); err != nil {
